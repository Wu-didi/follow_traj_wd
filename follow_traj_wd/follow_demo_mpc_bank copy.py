...
class VehicleTrajectoryFollower:
    ...
    def calc_ref_trajectory(self, state, cx, cy, cyaw, ck, sp, dl, pind):
        ...
        for i in range(T + 1):
            ...
            if (ind + dind) < ncourse:  #n course是路径点的总数，判断是否超出路径点的总数
                ...
                # dref[0, i] = 0.0
            # if (ind + i) < ncourse:
                xref[0, i] = cx[ind + dind * (i + 1)]
                xref[1, i] = cy[ind + dind * (i + 1)]
                xref[2, i] = sp[ind + dind * (i + 1)]
                xref[3, i] = cyaw[ind + dind * (i + 1)]
                dref[0, i] = 0.0
                # dref[0, i] = self.calculate_reference_steer(self.state, self.cyaw[ind + dind])
            else:
    ...
    def isinfront(self, x, y):
        ...
        angle_diff = (angle - self.state.yaw) * 180 / math.pi
        if angle > 180:
            angle_diff -= 360
    ...
    def calc_nearest_index(self, state, cx, cy, cyaw, pind):
        ...
        v = state.v if state.v > 2.7 else 2.7
        radius = DT * v
        dx = [state.x - icx for icx, icy in zip(cx, cy) if self.isinfront(icx, icy)]
        dy = [state.y - icy for icx, icy in zip(cx, cy) if self.isinfront(icx, icy)]
        d = [idx ** 2 + idy ** 2 for idx, idy in zip(dx, dy)]
        ind = bisect.bisect_left(d, radius ** 2) ## 找到第一个大于radius**2的索引
        # print(target_id, len(cx))
        mind = math.sqrt(d[ind]) # 最小距离
        dxl = cx[ind] - state.x
        dyl = cy[ind] - state.y
